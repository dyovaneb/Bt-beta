...
import { setSelectedMeshStateB3 } from "./B3Model";
 ...
 useEffect(() => {
    ...
    return <div className="interface">
        <div className="math-container"><MathJax inline>
            ...
            <p>Dada una habitación <span onClick={pintarMeshB3}  className="botonInterfaz">{"\\(\\mathcal{C}\\)"}</span>. Denotamos {"\\(\\psi \\in \\Psi \\)"} se llama <i>positiva con respecto a {"\\(\\mathcal C\\)"}</i> (respectivamente <i>negativa con respecto a {"\\(\\mathcal{C}\\)"}</i>) si {"\\(\\psi (x) {\\gt} 0\\)"} (respectivamente {"\\(\\psi (x) {\\lt} 0\\)"}) para todo {"\\(x \\in \\mathcal{C}\\)"}. Denotamos por {"\\(\\Psi (\\mathcal{C})^+\\)"} y {"\\(\\Psi (\\mathcal{C})^-\\)"} el conjunto de raíces afines positivas y negativas, respectivamente. Observamos que tenemos la unión disjunta {"\\(\\Psi = \\Psi (\\mathcal{C})^+ \\cup \\Psi (\\mathcal{C})^-\\)"}. </p>
